/*
 * @Author: @Guojufeng 
 * @Date: 2019-04-28 18:00:15 
 * @Last Modified by:   @Guojufeng 
 * @Last Modified time: 2019-04-28 18:00:15 
 */
/* global $ */
...
function ajaxListData(params) {
  $.ajax({
        ...
        if (data.result == 1) {
          ...
          if (len > 0) {
            ...
            if (HasNextPage) { //总页数-当前页数可判断是否还有下一页
              ...
              $('.list-more').text('正在加载').addClass('show');
            } else {
              canIload = false;
              $('.list-more').text('').removeClass('show');
            }
            bodyH = $('body').height() || $('html').height() || $(document).height(); //获取总内容长度
          } else {
            // 空数据
    ...
    error: function (err) {
      ...
      $('.loading').hide();
      $('.rank_nodate').show().text('数据加载失败');
      console.log('error回调：',err.status, err.statusText, err);
    }
  });
}
